...
<%-- <form action="${pageContext.request.contextPath}/score/scorelist" method="post">
	<c:forEach items="${centerName}" var="center">
		...
		<input type="button" name="centerName" class="pinkButton firstCenter" value="${center.centerName}" onclick="${pageContext.request.contextPath}/score/scorelist" />
	</c:forEach>
</form> --%>
		...
		<ul class="pagination pageModal">
			...
			<li><c:if test="${pager.groupNo>1}">
					<a class="innerPager arrow left"
						href="scorelist?pageNo=${pager.startPageNo-1}&centerCode=${param.centerCode}">이전</a>
				</c:if></li>
			...
			<li><c:if test="${pager.groupNo<pager.totalGroupNo}">
					<a class="innerPager arrow right"
						href="scorelist?pageNo=${pager.endPageNo+1}&centerCode=${param.centerCode}">다음</a>
				</c:if></li>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="/score/insertScore" method="post" >
					...
					<input type="hidden" name="checkSeason" value="${season}" />
						...
						<c:forEach items="${usingCodeList}" var="usingCodeList">
							<tr>
								...
								<td><input type="hidden" name="arrayCheckGroupCode"
									value="${usingCodeList.checkGroupCode}"> <input
									type="hidden" name="arrayCheckDetailCode"
